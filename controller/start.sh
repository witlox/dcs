...
docker run --name fs -v /tmp/store:/tmp/store -d witlox/store
...
docker run -p 80:80 --name web --link elk:elk --link ilm:ilm --link wjc:wjc --link fs:fs -v $(pwd)/nginx.conf:/etc/nginx/nginx.conf:ro -d nginx

...
docker run --name store -v $(pwd)/store:/var/www/app:ro -v /tmp/store:/tmp/store -d witlox/store
...
docker run --name ilm --link db:db --link elk:elk -v $(pwd)/ilm:/var/www/app:rw -v $(pwd)/ilm.conf:/var/www/app/ilm.conf:ro -d witlox/ilm
#docker run --name wjc --link db:db --link elk:elk -v $(pwd)/wjc.conf:/var/www/app/wjc.conf:ro -d witlox/wjc
docker run --name wjc --link db:db --link elk:elk -v$ (pwd)/wjc:/var/www/app:rw -v $(pwd)/wjc.conf:/var/www/app/wjc.conf:ro -d witlox/wjc
